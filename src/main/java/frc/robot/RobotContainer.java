...
public class RobotContainer {
  ...
  public final PivotSubsystem pivot =
      new PivotSubsystem(new PivotIOTalonFX());

  public final RollerSubsystem roller =
      new RollerSubsystem(new RollerIOTalonFX());

  public final ShooterSubsystem shooter =
      new ShooterSubsystem(new ShooterIOTalonFX());

  public final HoodSubsystem hood =
      new HoodSubsystem(new HoodIOTalonFXS());

  public final NeckSubsystem neck =
      new NeckSubsystem(new NeckIOTalonFX());

  public final TransporterSubsystem transporter =
      new TransporterSubsystem(new TransporterIOTalonFX());

  public final MagicFloorSubsystem magicFloor =
      new MagicFloorSubsystem(new MagicFloorIOTalonFX());

  public final TargetingSubsystem targeting =
      new TargetingSubsystem(drivetrain);
  ...
  private void configureBindings() {
    ...
    // Old leftDriveController.getLeftThumb()
    // -> Static Shot
    driverController
        .leftStick()
        .whileTrue(
    ...
    // Old leftDriveController.getBottomThumb()
    // -> Hail Mary
    driverController
        .leftBumper()
        .whileTrue(
    ...
    // Old leftDriveController trigger
    // -> Simple Align and Shoot
    driverController
        .leftTrigger()
        .whileTrue(
            Commands.defer(
    ...
    // Old rightDriveController trigger
    // -> Roller intake
    driverController
        .rightTrigger()
        .whileTrue(roller.setVoltage(11));

    /*
     *
     * Old rightDriveController POV
     * -> Xbox D-pad
     */
    ...
    // Old rightDriveController bottom thumb
    // -> Dynamic Shot
    driverController
        .rightStick()
        .whileTrue(
    ...
    /*
     * Old rightDriveController left-bottom-right
     * -> Hood preset
     *
     * Xbox B
     */
    driverController
        .b()
        .onTrue(
            hood.setHoodAngleForever(
                () -> Rotation2d.fromRotations(.101)));

    /*
     * Old leftDriveController left-bottom-middle
     * -> transporter offset +.5
     *
     * Xbox Y
     */
    driverController
    ...
    /*
     * Old leftDriveController left-bottom-right
     * -> transporter offset -.5
     *
     * Xbox X
     */
    ...
    /*
     * Old leftDriveController left-top-left
     * -> reset pose
     *
     * Xbox Start
     */
    ...
    /*
     * Old leftDriveController left-top-middle
     * -> toggle intake
     *
     * Xbox Back
     */
    ...
    operatorController
        .getA()
        .onTrue(roller.setVoltage(-12));

    operatorController
        .getB()
        .whileTrue(neck.setNeckRPSForever(-20));
    ...
    operatorController
        .getLeftTrigger()
        .whileTrue(
            shooter.setShooterRPSForever(-20));
  ...
  private double getXVelocity() {
    return DriveConstants.MAX_TRANSLATIONAL_SPEED.in(
            MetersPerSecond)
  ...
  private double getThetaVelocity() {
    return DriveConstants.MAX_ROTATIONAL_SPEED.in(
            RadiansPerSecond)
