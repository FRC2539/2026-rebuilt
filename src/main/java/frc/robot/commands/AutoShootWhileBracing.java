...
public class AutoShootWhileBracing extends Command {
  ...
  public AutoShootWhileBracing(
    ...
    SwerveRequest.FieldCentricFacingAngle aimRequest =
    ...
    SwerveRequest.SwerveDriveBrake brakeRequest =
        ...
        Commands.run(
                () ->
                    drivetrain.setControl(
                        aimRequest.withVelocityX(0)
                            .withVelocityY(0)
                            .withTargetDirection(
                                targeting.getIdealRobotHeading().get())),
                drivetrain)
            .until(
                () -> {
                  ...
                  double error =
                      MathUtil.angleModulus(target - current);
    ...
    Command waitUntilReady =
        Commands.waitUntil(
            () -> shooter.isAtSetpoint() && hood.isAtSetpoint());
